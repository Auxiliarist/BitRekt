...
namespace BitMexAPI.Websocket
{
    public class BitmexWebsocketCommunicator : IDisposable
    {
        ...
        private readonly Subject<WebsocketReconnectionType> _reconnectionSubject = new Subject<WebsocketReconnectionType>();

        public BitmexWebsocketCommunicator(Uri url, Func<ClientWebSocket> clientFactory = null)
        {
            BitmexValidation.ValidateInput(url, nameof(url));
        ...
        /// </summary>
        public IObservable<WebsocketReconnectionType> ReconnectionHappened => _reconnectionSubject.AsObservable();
        ...
        public Task Start()
        {
            ...
            return StartClient(_url, _cancelation.Token, WebsocketReconnectionType.Initial);
        }
        ...
        {
            BitmexValidation.ValidateInput(message, nameof(message));
        ...
        private async Task StartClient(Uri uri, CancellationToken token, WebsocketReconnectionType type)
        {
            DeactiveLastChance();
...
#pragma warning restore 4014               
            ...
            catch (Exception e)
            {
                ...
                await Reconnect(WebsocketReconnectionType.Error);
            }
        }
        ...
        private async Task<ClientWebSocket> GetClient()
        {
            ...
            {
                await Reconnect(WebsocketReconnectionType.Lost);
            }
            return _client;
        }

        private async Task Reconnect(WebsocketReconnectionType type)
        {
            if (_disposing)
                return;
        ...
        private async Task LastChance(object state)
        {
            ...
            if (diffMs > timeoutMs)
            {
                ...
                _client?.Abort();
                _client?.Dispose();
                await Reconnect(WebsocketReconnectionType.NoMessageReceived);
            }
        }
